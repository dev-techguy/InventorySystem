...
public class MedicineStore {
    ...
    final ImageIcon iconStore = new ImageIcon(url4);
    ...
    //components
    JLabel llogo, llogo1, llogo2, llogo3, lstock, lcost, ltotalcost, lname, lserialnumber, ledate, lsection, lprescription, ldistributer, lcategory, ltcost, ltstock, lstoretax, ltax, lbuyingprice;
    JTable table;
    JTextField tstoresearch, tstock, tcost, ttotalcost, tname, tserialnumber, tedate, tsection, tprescription, tdistributer, tcategory, ttcost, ttstock, tstoretax, ttax, tbuyingprice;
    ...
    //set variables
    String[] values = new String[]{"Product_Name", "Serial Number", "Available Stock", "Buying Price", "Selling Price", "Total Tax", "Total Cost", "Expiry Date", "Under/Section", "Prescription", "Distributer", "Category", "Report", "Created On", "Updated On"};
    ...
    private void tablecont() {
        ...
        double storeBuying = Double.parseDouble(String.valueOf(table.getValueAt(table.getSelectedRow(), 3)));
        double storecost = Double.parseDouble(String.valueOf(table.getValueAt(table.getSelectedRow(), 4)));
        double storetax = Double.parseDouble(String.valueOf(table.getValueAt(table.getSelectedRow(), 5)));
        double storetotalcost = Double.parseDouble(String.valueOf(table.getValueAt(table.getSelectedRow(), 6)));
        String storeedate = table.getValueAt(table.getSelectedRow(), 7).toString();
        String storesection = table.getValueAt(table.getSelectedRow(), 8).toString();
        String storeprescription = table.getValueAt(table.getSelectedRow(), 9).toString();
        String storedistributer = table.getValueAt(table.getSelectedRow(), 10).toString();
        String storecategory = table.getValueAt(table.getSelectedRow(), 11).toString();
        String storeport = table.getValueAt(table.getSelectedRow(), 12).toString();
        ...
        tbuyingprice.setText(String.format("%.2f", storeBuying));
        tcost.setText(String.format("%.2f", storecost));
        ttax.setText(String.format("%.2f", storetax));
    ...
    private void reupload() {
        ...
        try {
            ...
            if (rs.next()) {
                do {
                    ...
                    double productBuying = rs.getDouble("BuyingPrice");
                    String buyingPrice = String.format("%.2f", productBuying);
                    double drugcost = rs.getDouble("Mcost");
                    String scost = String.format("%.2f", drugcost);
                    double drugtax = rs.getDouble("TotalTax");
                    String stax = String.format("%.2f", drugtax);
                    ...
                    pharmacymodel.addRow(new String[]{drugname, drugserial, sstock, buyingPrice, scost, stax, stcost, drugdate, drugsection, drugp, drugdistributer, drugcategory, drugreport, drugredited, drugupdate});
                ...
                sbload.setEnabled(true);
                supadete.setEnabled(true);
            } else {
                Toolkit.getDefaultToolkit().beep();
                table.setModel(pharmacymodel);
                JOptionPane.showMessageDialog(null, "No Stock In The Store", "Notification", JOptionPane.ERROR_MESSAGE);
                rs.close();
                stmt.close();
                con.close();
                ...
                sbload.setEnabled(false);
                supadete.setEnabled(false);
            }
        } catch (SQLException x) {
            xamppfailure.getCon();
            reupload();
        ...
        try {
            ...
            rs = prs.executeQuery();
            if (rs.next()) {
                double taxfound = rs.getDouble("STORETAX");
                String sstax = String.format("%.2f", taxfound);

                tstoretax.setText(sstax);
                rs.close();
                stmt.close();
                con.close();
            }
    ...
    public void MStore() {
        ...
        table.setPreferredScrollableViewportSize(new Dimension((int) (screenSize.width / 1.45), (int) (screenSize.height / 1.38)));
        table.revalidate();
        ...
        llogo = new JLabel("PRODUCT(S) STORE");
        ...
        lbuyingprice = new JLabel("Buying Price [KES]");
        lbuyingprice.setFont(new Font("Tahoma", Font.PLAIN, 15));
        lcost = new JLabel("Selling Price [KES]");
        ...
        ltax = new JLabel("Total Tax Of Item [KES]");
        ltax.setFont(new Font("Tahoma", Font.PLAIN, 15));
        ltotalcost = new JLabel("Total Cost Of Item [KES]");
        ...
        ltcost = new JLabel("Total Cost [KES]");
        ltcost.setFont(new Font("Tahoma", Font.PLAIN, 15));
        lstoretax = new JLabel("Total Tax [KES]");
        lstoretax.setFont(new Font("Tahoma", Font.PLAIN, 15));

        treport = new JTextArea(3, 16);
        ...
        tstock.setBackground(Color.LIGHT_GRAY);
        tstock.setEditable(false);
        tbuyingprice = new JTextField(15);
        tbuyingprice.setFont(new Font("Tahoma", Font.PLAIN, 13));
        tbuyingprice.setEditable(false);
        tbuyingprice.setBackground(Color.LIGHT_GRAY);
        ...
        ttax = new JTextField(15);
        ttax.setFont(new Font("Tahoma", Font.PLAIN, 13));
        ttax.setEditable(false);
        ttax.setBackground(Color.LIGHT_GRAY);
        ...
        tstoretax = new JTextField(10);
        tstoretax.setBackground(Color.LIGHT_GRAY);
        tstoretax.setFont(new Font("Tahoma", Font.PLAIN, 13));
        ...
        v.gridy++;
        panelstoremenu.add(lbuyingprice, v);
        v.gridy++;
        panelstoremenu.add(tbuyingprice, v);
        v.gridy++;
        ...
        v.gridy++;
        panelstoremenu.add(ltax, v);
        v.gridy++;
        panelstoremenu.add(ttax, v);
        v.gridy++;
...
//        panelstoremenu.add(sdelete, v);
        ...
        v.gridy++;
        panelstorelogo.add(lstoretax, v);
        v.gridy++;
        panelstorelogo.add(tstoretax, v);
        v.gridy++;
...
//        });
        ...
        sbsearch.addActionListener(e -> {
            ...
            if (storesearch.equalsIgnoreCase("")) {
                JOptionPane.showMessageDialog(null, "Please Enter Either Of this:" + "\n" + "Product Serial Number" + "\n"
                        + "Product Name" + "\n" + "Product Arrangement " + "\n" + "Product Category " + "\n" + "Product Distributer" + "\n\n" + "\t\t"
                        + "For Search", "Store Message", JOptionPane.INFORMATION_MESSAGE, iconStore);
            } else {
                DefaultTableModel pharmacymodel = new DefaultTableModel();
                //dm.addColumn("Medicine Name");
                pharmacymodel.setColumnIdentifiers(values);
                ...
                try {
                    ...
                    if (rs.next()) {
                        do {
                            ...
                            ttax.setText(String.format("%.2f", drugtax));
        ...
        supadete.addActionListener(event -> {
            ...
            String storetcost = tcost.getText();
            double newPrice = Double.parseDouble(storetcost);
            String buying = tbuyingprice.getText();
            double buyinPrice = Double.parseDouble(buying);
            String Quantity = tstock.getText();
            double productQuantity = Double.parseDouble(Quantity);
            String totalCost = ttotalcost.getText();
            double storePrice = Double.parseDouble(totalCost);
            ...
            if (storename.equalsIgnoreCase("") && storeserial.equalsIgnoreCase("") && storetcost.equalsIgnoreCase("")
                ...
                try {
                    ...
                    String sqlupdate = "UPDATE store set Mname = '" + storename + "', Mquantity = '" + productQuantity + "', BuyingPrice = '" + buyinPrice + "'"
                            ...
                            + ",Mcategory = '" + storecategory + "',Mreport = '" + storereport + "',UpdatedOn = '" + fileeditedlast + "' WHERE Mserial ='" + storeserial + "'";
                    String[] option = {"Yes", "No"};
                    int selloption = JOptionPane.showOptionDialog(null, "Proceed in Updating" + " " + storename + " " + "Details", "Update Confirmation", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, iconStore, option, option[1]);
                    if (selloption == 0) {
