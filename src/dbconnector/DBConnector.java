...
public class DBConnector extends Component {
    ...
    URL urlxamppstop = Main.class.getResource("/BackUpFile/Stop.bat");
...
//                BufferedWriter fileOut;
                ...
                fc.setDialogTitle("Database BackUp");
...
//                    fileOut = new BufferedWriter(new FileWriter(String.valueOf(urlbackup)));
//                    fileOut.write(path);
//
...
//                    System.out.println(fileOut);
                    ...
                    try {
                        ...
                        //Process runtimeprocess = Runtime.getRuntime().exec("cmd.exe " + "/c" + " start /B " + urlbackup + " " + new File(path));
            ...
            Toolkit.getDefaultToolkit().beep();
            int dbstate = JOptionPane.showOptionDialog(null, "Database Connection Failure\nBackup Can't Be done\n\nTry To Start Database Connection", "BackUp Notification", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, icon2, option, option[1]);
            if (dbstate == 0) {
                restartXampp();
            }
            if (dbstate == 1) {
    ...
    public void getCon() {
        String[] option = {"Retry", "Ok"};
        Toolkit.getDefaultToolkit().beep();
        int dbstate = JOptionPane.showOptionDialog(null, "Database Connection Failure\nRetry Starting The Connection\n\nAnd Try Again", "Notification", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, icon2, option, option[1]);
        if (dbstate == 0) {
            restartXampp();
        }
        if (dbstate == 1) {
