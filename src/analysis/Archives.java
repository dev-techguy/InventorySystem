...
public class Archives {
    ...
    final ImageIcon iconPayments = new ImageIcon(url2);
    final ImageIcon iconPastRecords = new ImageIcon(url3);
    ...
    //set variables
    String[] values = new String[]{"Product Name", "Serial Number", "Stock In", "Tax In", "Stock In Cost", "Stock Out", "Tax Out", "Stock Out Cost", "Total Profit", "Total Deficit", "Saved On"};
    ...
    /**
     * Function for calculating the sum payments
     */
    private void receiveSearchData(String searchDate, String searchDetails) {
        try {
            con = DBConnector.getConnection();
            String sqlqsum1 = null;
            if (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty() && tsearch2.getText().equalsIgnoreCase(searchDetails) && !tsearch2.getText().equalsIgnoreCase("")) {
                //searching with textfield only
                sqlqsum1 = "SELECT SUM(Tcost) TOBEPAID,SUM(Payed) PAIDAMOUNT,SUM(Balance) USERBALANCE FROM paymentrecords WHERE Cname = '" + tsearch2.getText() + "' || CIdentity = '" + tsearch2.getText() + "'";
                prs = con.prepareStatement(sqlqsum1);
                rs = prs.executeQuery();
            }
            if (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase(searchDate) && tsearch2.getText().equalsIgnoreCase("") && !((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty()) {
                //searching only with date
                sqlqsum1 = "SELECT SUM(Tcost) TOBEPAID,SUM(Payed) PAIDAMOUNT,SUM(Balance) USERBALANCE FROM paymentrecords WHERE Dpayed = '" + searchDate + "'";
                prs = con.prepareStatement(sqlqsum1);
                rs = prs.executeQuery();
            }
            if ((!((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty() && !tsearch2.getText().equalsIgnoreCase("")) && (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase(searchDate) && tsearch2.getText().equalsIgnoreCase(searchDetails))) {
                //searching with both entries that is date and textfield
                sqlqsum1 = "SELECT SUM(Tcost) TOBEPAID,SUM(Payed) PAIDAMOUNT,SUM(Balance) USERBALANCE FROM paymentrecords WHERE (Cname = '" + tsearch2.getText() + "' || CIdentity = '" + tsearch2.getText() + "') && (Dpayed = '" + searchDate + "')";
                prs = con.prepareStatement(sqlqsum1);
                rs = prs.executeQuery();
            }
            /**
             * Check if the execution is correct
             * */
            if (rs.next()) {
                double payableAmount = rs.getDouble("TOBEPAID");
                String payable = String.format("%.2f", payableAmount);
                double paymentsfound = rs.getDouble("PAIDAMOUNT");
                String userpayments = String.format("%.2f", paymentsfound);
                double balancefound = rs.getDouble("USERBALANCE");
                String userbalance = String.format("%.2f", balancefound);

                ttotalbalance.setText(userbalance);
                ttotalpatyments.setText(userpayments);
                ttotalamount.setText(payable);
     ...
     */
    private void getToTable(String paidDate) {
        ...
        try {
            ...
            if ((!((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty() && !tsearch2.getText().equalsIgnoreCase("")) && (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase(paidDate) && tsearch2.getText().equalsIgnoreCase(tsearch2.getText()))) {
                ...
                stmt = con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
                rs = stmt.executeQuery(fetchTextrecord);
                //rs = stmt.executeQuery(fetchrecordTwo);
            }
            if (rs.next()) {
                do {
                 ...
                 * Call this function
                 * */
                receiveSearchData(paidDate, tsearch2.getText());

            } else {
                table2.setModel(pharmacymodel);
                Toolkit.getDefaultToolkit().beep();
                JOptionPane.showMessageDialog(null, "Sorry No Payment Record(s) Found For" + " " + tsearch2.getText() + " on date " + paidDate, "Notification", JOptionPane.INFORMATION_MESSAGE);
                uplaod2();
                rs.close();
                stmt.close();
                con.close();
    ...
    /**
     * Payment for workers search
     */
    private void searchingPayments() {
        ...
        if (valuesearch.equalsIgnoreCase("") && ((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty()) {
            JOptionPane.showMessageDialog(null, "Please Enter :: \nThe Record Name\nIdentity\nSelect Date or Use Both For Search", "Archive Message", JOptionPane.INFORMATION_MESSAGE, iconPayments);
        } else {
            if (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase(medate) && tsearch2.getText().equalsIgnoreCase("") && !((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase("")) {
                //pass date to the table model
                getToTable(medate);
            } else if (((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty() && tsearch2.getText().equalsIgnoreCase(valuesearch) && !valuesearch.equalsIgnoreCase("")) {
                //pass date to the table model
                getToTable(medate);
            } else if ((((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().equalsIgnoreCase(medate) && tsearch2.getText().equalsIgnoreCase(valuesearch)) && (!valuesearch.equalsIgnoreCase("") && !((JTextComponent) dateexpry.getDateEditor().getUiComponent()).getText().isEmpty())) {
                //pass date to the table model
                getToTable(medate);
            }
        }
    ...
    private void uplaod2() {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                //do this
                bsearch2.setEnabled(true);
                bload2.setEnabled(true);
                bdelete2.setEnabled(true);
            } else {
                table2.setModel(pharmacymodel);
                Toolkit.getDefaultToolkit().beep();
                JOptionPane.showMessageDialog(null, "No Payment Records In The Store For Analysis", "Notification", JOptionPane.ERROR_MESSAGE);
                rs.close();
                stmt.close();
                con.close();

                //do this
                bsearch2.setEnabled(false);
    ...
    private void searchengine() {
        ...
        if (valuesearch.equalsIgnoreCase("") || valuesearch.equalsIgnoreCase(null)) {
            JOptionPane.showMessageDialog(null, "Please Enter The Record Name/Serial Number", "Search Message", JOptionPane.INFORMATION_MESSAGE,iconPastRecords);
        } else {
            DefaultTableModel pharmacymodel = new DefaultTableModel();
            //dm.addColumn("Medicine Name");
            ...
            try {
                ...
                if (rs.next()) {
                    do {
                        ...
                        pharmacymodel.addRow(new String[]{drugname, drugserial, sinstock, taxin, sincost, soutstock, taxout, soutcost, sprofit, sloss, drugeditedon});
    ...
    private void uplaod() {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                Toolkit.getDefaultToolkit().beep();
                table.setModel(pharmacymodel);
                JOptionPane.showMessageDialog(null, "No Past Records In The Store For Analysis", "Notification", JOptionPane.ERROR_MESSAGE);
                rs.close();
                stmt.close();
                con.close();
    ...
    public void archives() {
        ...
        table.setPreferredScrollableViewportSize(new Dimension((int) (screenSize.width / 1.41), (int) (screenSize.height / 1.40)));
        table.revalidate();
        ...
        table2.setPreferredScrollableViewportSize(new Dimension((int) (screenSize.width / 1.45), (int) (screenSize.height / 1.40)));
        table2.revalidate();
        ...
        ltotalamount = new JLabel("Total Amount[KES]");
        ...
        ltotalpatyments = new JLabel("Total Payments[KES]");
        ...
        ltotalbalance = new JLabel("Total Balance[KES]");
